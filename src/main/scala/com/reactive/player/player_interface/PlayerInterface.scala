...
import com.reactive.player.player_interface.state.{InterfaceState, InterfaceStateFactory}
...
class PlayerInterface[QC <: QueueCommand] private (
  ...
  private def getNextBehavior(queue: ActorRef[QC], reply: QueueReplies.Reply): Behavior[Command] = {
